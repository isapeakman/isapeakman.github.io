...
* 【消息顺序队列】`List`类型，按照消息插入顺序存储。核心作用有三个：1) 支持`contains()`、`remove(Object)`、`readAll()`等List接口方法，因为ZSet中存储的是包含randomId的完整结构体，无法直接按业务值查找；2) 保持消息的插入顺序，`readAll()`返回的是插入顺序而非到期时间顺序；3) 支持相同业务值的重复插入，通过randomId区分不同消息
...
- `if v[1] == value then redis.call('publish', KEYS[4], ARGV[1]);`如果当前插入的消息成为了ZSet中最早到期的消息（即ZSet的第一个元素），则发布到期时间通知
...
针对以上消息丢失的场景，可以采取以下策略来保障消息的可靠性：

1. **本地消息表 + 定时任务兜底**（推荐）：
   - 生产者在发送延迟消息的同时，将消息记录到数据库的本地消息表中，并标记状态为"待处理"
   - 启动一个定时任务（如每分钟）扫描本地消息表，检查超过一定时间（如5分钟）仍未处理的消息
   - 如果消息未被处理，则重新发送到延迟队列或直接执行业务逻辑

2. **Redis持久化配置**：
   - 确保Redis启用了AOF持久化，并设置`appendfsync everysec`（至少每秒刷盘一次）
   - 配置合理的RDB快照策略，定期备份数据

3. **最终一致性方案**：
   - 对于金融级核心链路（如支付超时关闭），不建议完全依赖Redis延迟队列
   - 采用"MQ延迟消息 + 定时任务扫表"的双保险方案，即使MQ没消费到，定时任务也能兜底处理

### 生产者下线后消息转移问题

当生产者在消息到期前下线，如果消费者也没有初始化延迟队列（即没有调用`RedissonClient.getDelayedQueue(queue)`），会导致过期消息一直停留在消息延迟列表(ZSet)中，无法被转移到目标消息列表(List)，从而无法被消费者获取和处理。

**解决方案**：确保至少有一个活跃的客户端（生产者或消费者）初始化了`RDelayedQueue`，这样才能触发消息转移机制。建议在消费者端初始化延迟队列，确保消息转移的可靠性。

### 重复消费问题分析

1. **多个客户端都在尝试转移，但同一条消息大多数情况不会被重复转移到目标队列中**
...
**除了Lua执行过程Redis突然宕机可能才会导致消息重复转移到目标队列中。**

2. **多个消费者从目标队列取出同一个消息时，也不会重复消费**

消费者从目标队列获取消息使用的是`BLPOP`（阻塞弹出）命令，这是Redis原生支持的原子操作。当多个消费者同时监听同一个目标队列时，`BLPOP`会保证同一个消息只会被一个消费者获取。即使多个消费者同时收到通知，Redis的单线程特性也确保了只有一个消费者能成功弹出消息。

### Lua脚本执行期间Redis宕机的重复消费风险

Redis官方文档明确说明：Lua脚本要么全部执行，要么全部不执行。如果脚本执行过程中Redis崩溃，脚本的所有效果都会被回滚。

但是，有一种极端情况需要注意：如果Redis启用了AOF持久化，并且设置了`appendfsync everysec`（每秒刷盘），那么：

- Lua脚本中的写操作（如`RPUSH`、`ZREM`）可能已经被写入AOF缓冲区
- 在AOF刷盘之前Redis崩溃
- 重启后，AOF重放会恢复已经写入缓冲区的操作，但尚未写入的操作则不会被恢复

这种情况下，可能出现消息已经放入目标队列但尚未从延迟队列移除的情况，导致重启后同一消息被重复放入目标队列。

虽然Redis宕机概率极小，且Lua脚本执行速度极快（通常在毫秒级别），但**业务端仍需做好幂等性设计**，以应对这种极端场景。
