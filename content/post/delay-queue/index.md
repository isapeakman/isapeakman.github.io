...
## 基于`DelayedQueue`实现订单延迟关闭/超时取消

### 常见方案及比对

一是RocketMQ虽然是本身支持延迟发送功能，但一旦消息发生堆积了。消息达不到指定的延时，而Redisson的方式比MQ的方式性能更叫高效。

二是现在的项目中都会依赖Redis，但并不都会依赖MQ，也是为了减少对中间件的依赖
...
3. **如果只有 Redis 且业务允许极小概率不一致**：用 Redisson，别自己造轮子。
